...
parser = argparse.ArgumentParser(description="Run SuPreMo with optional track parameters for Enformer scores")
parser.add_argument("input_file", type=str, help="Input file for variants")
parser.add_argument("--dir", type=str, required=True, help="Directory path for output")
parser.add_argument("--file", type=str, required=True, help="Output filename")
parser.add_argument("--get_Enformer_scores", action="store_true", help="Flag to get Enformer scores")
parser.add_argument("--track", type=str, help="Specify track to use")
parser.add_argument("--protocol", type=str, help="Specify protocol to use")
parser.add_argument("--cell_type", type=str, help="Specify cell type to use")
...
# Updated get_scores call within SuPreMo
if args.get_Enformer_scores:
    # Call get_scores with additional arguments for track, protocol, and cell_type
    scores = get_Enformer_scores_utils.get_scores(
        POS, SVTYPE, SVLEN, sequences, scores_to_use, shift, revcomp, get_tracks, 
        cell_type=args.cell_type, protocol=args.protocol, binding_factor=args.track
    )
